...
export function getAdvancedFeaturesForEngine(engine: string): {
...
export function getEnabledAdvancedFeatures(engine: string): {
  [feature: string]: boolean;
} {
...
export function getCustomEngines(): CustomEngineConfig[] {
  try {
    const raw = getPref("customEngines" as any) as string;
...
export function setCustomEngines(engines: CustomEngineConfig[]) {
  setPref("customEngines" as any, JSON.stringify(engines) as any);
}

export function addCustomEngine(engine: CustomEngineConfig) {
  const engines = getCustomEngines();
  engines.push(engine);
  setCustomEngines(engines);
}

export function updateCustomEngine(engine: CustomEngineConfig) {
  const engines = getCustomEngines();
...
export function deleteCustomEngine(engineId: string) {
  const engines = getCustomEngines();
  setCustomEngines(engines.filter((e) => e.id !== engineId));
}

export function getCustomEngineById(
  engineId: string,
): CustomEngineConfig | undefined {
  return getCustomEngines().find((e) => e.id === engineId);
}
