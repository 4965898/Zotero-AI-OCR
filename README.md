...
| 引擎             | 说明             | API 模式 | 高级功能                                                                         |
| ---------------- | ---------------- | -------- | -------------------------------------------------------------------------------- |
| PP-OCRv6         | 基础文字识别     | 异步     | 方向矫正、扭曲矫正、文本行方向                                                   |
...
- 🐛 **关闭/剥离模型思考内容** — 修复思考型模型（豆包、InternVL、Qwen3、GLM-4.5+ 等）把推理过程混入 OCR 结果的问题，双层处理：
  - **请求层关闭思考**：对支持该参数的平台自动附加关闭思考参数（豆包/智谱 `thinking: {type: "disabled"}`、阿里/硅基流动 `enable_thinking: false`、OpenRouter `reasoning: {enabled: false}`）；若平台/模型不支持该参数导致请求失败，自动去掉参数重试一次
  - **响应层剥离兜底**：所有引擎（含不支持关闭参数的平台与自定义引擎）的识别结果统一剥离思考标签（完整块、未闭合尾随块）并解开 answer 标签包裹；Gemini 格式额外过滤带 thought 标记的思考 parts
  - 附带收益：关闭思考后响应更快、token 消耗更少
...
- 🐛 **修复"识别指定页面"返回空内容** — 修复 PaddleOCR 引擎识别指定页面后，笔记仅有页码标题而无正文的问题
  - **根因一（加密 PDF）**：读秀/超星等来源的扫描 PDF 通常带有空口令加密，pdf-lib 无法解密抽页；此前抽页失败会被静默回退到逐页图片渲染，导致用户无感知地拿到空结果。现已在抽页前检测加密状态，加密 PDF 自动改传原始完整 PDF（服务端可正常解密识别），按页返回后再过滤出请求页码
  - **根因二（空白渲染）**：页面渲染画布原先创建在阅读器外层窗口，与 pdf.js 分属不同 compartment，跨安全边界传参导致渲染出空白图片。现改为在 pdf.js 所在 iframe 内创建画布（同 compartment），从根源修复
  - **三层递进策略**：抽取指定页生成临时 PDF 上传 → 加密 PDF 或结果无文本时自动改传原始完整 PDF 并按页码过滤 → 逐页渲染仅作最后兜底，任何一种 PDF 都能拿到可靠结果
- ⚡ **指定页识别性能优化** — PaddleOCR 引擎由每页一次 API 调用合并为整份一次调用，减少排队等待与配额消耗
- 📝 此前版本的"识别指定页面"依赖 pdf.js 跨 compartment 渲染，对部分 PDF（尤其加密扫描件）会静默产出空白图片，本版本彻底重构了该链路
...
- 🆕 **PP-OCRv6 支持** — 升级 PaddleOCR 引擎至最新 PP-OCRv6 模型，替换原有 PP-OCRv5
